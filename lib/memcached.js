...
(function (nMemcached) {
  ...
  memcached.command = function memcachedCommand(queryCompiler, server) {
      ...
      this.activeQueries++;
      let query = toString.call(queryCompiler) === '[object Function]' ? queryCompiler() : queryCompiler;
      ...
      let redundancy = this.redundancy && this.redundancy < this.servers.length
  ...
  privates.buffer = function BufferBuffer(S, BufferStream) {
    ...
    // response ending
    let buffer = S.responseBuffer;
    const length = buffer.length;
    if (buffer[length - 2] === LINEBREAK[0] && buffer[length - 1] === LINEBREAK[1]) {
      buffer = Utils.reallocString(buffer);

      const chunks = buffer.split(LINEBREAK);
      ...
      if (chunks[chunkLength].length === 0) chunks.pop();
  ...
  memcached.rawDataReceived = function rawDataReceived(S) {
    ...
    while(S.bufferArray.length && privates.allCommands.test(S.bufferArray[0])) {
      ...
      if (command === 'VALUE' && S.bufferArray.indexOf('END') === -1) {
        return S.bufferArray.unshift(token);
      }

      // check for dedicated parser
      if (privates.parsers[command]) {
        ...
        resultSet = privates.parsers[command].call(S, tokenSet, dataSet || token, err, queue, this);
  ...
  privates.setters = function setters(type, validate, key, value, lifetime, callback, cas) {
    ...
    this.command(function settersCommand(noreply) {
      const noReply = noreply || !hasCallback;
      return {
          key: fullkey
        , callback: callback
        , lifetime: lifetime
